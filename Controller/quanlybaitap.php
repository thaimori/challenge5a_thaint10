...
// Check if file already exists
    ...
    $target_dir2 = "../uploads/nopbai/".$_SESSION['user_id'];
    ...
    if ($nopbaiOk == 0) {
        ...
        if (move_uploaded_file($_FILES["filenop"]["tmp_name"], $target_file2)) {
            ...
            //echo $baitap_id;
            $nopbaitap->createNopBaitap($baitap_id,$_SESSION['user_id'] , $target_file2, $ngaynop);
            $allBaitap = $baitap->getAllBaitap();
...
} elseif (isset($_GET['btid'])) {
    $danhsach = $nopbaitap->getNopBaitap($_GET['btid']);
    //echo $danhsach['username'];
    include_once("../View/quanlynopbaitap.html");
} else {
    
    $allBaitap = $baitap->getAllBaitap_Nopbai_adm();
    $allNopBaitap = $baitap->getAllBaitap_Nopbai();
